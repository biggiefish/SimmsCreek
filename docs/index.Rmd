...
During each daily trap visit, volunteers record measures of water temperature, pH, total suspended sediment (TSS), dissolved oxygen (DO), and water level. Mean annual measures of each parameter are presented below for the spring and fall monitoring periods (Table 3 and Table 4, Figure 2). 
...
annual.catches.fall.CO.old <- catch_plot.data %>% 
                                  filter(Period == "Fall", Species == "CO", Year <2022) %>%
...
Since 2008, a total of `r format(sum(catch_annual.Table.Spring$nCT), big.mark =",")` juvenile CCT and `r format(sum(catch_annual.Table.Spring$nCO), big.mark =",")` juvenile Coho Salmon have been captured during spring fence operation. Annual spring captures of CCT have ranged from `r format(min(catch_annual.Table.Spring$nCT), big.mark =",")` to `r format(max(catch_annual.Table.Spring$nCT), big.mark =",")` (mean = `r format(annual.catches.spring.CT$mean, digits = 1)` $\pm$ `r format(annual.catches.spring.CT$SD, digits = 1)`) while spring CO captures have varied from `r format(min(catch_annual.Table.Spring$nCO), big.mark =",")` to `r format(max(catch_annual.Table.Spring$nCO), big.mark =",")` with an overall mean of `r format(mean(catch_annual.Table.Spring$nCO), digits = 1)` fish (Figure 3 and Figure 4, Table 5 and Table 6).

Far fewer fish are captured during fall fence operations. Total annual CCT catches range from `r annual.catches.fall.CT$min` to `r annual.catches.fall.CT$max` (mean = `r annual.catches.fall.CT$mean` $\pm$ `r round(annual.catches.fall.CT$SD,1)`), while returning adult CO have ranged in number from  `r annual.catches.fall.CO$min`to `r annual.catches.fall.CO$max`(mean =`r annual.catches.fall.CO$mean` $\pm$ `r round(annual.catches.fall.CO$SD,1)`). However, CO returns prior to 2022 were considerably lower, ranging from `r annual.catches.fall.CO.old$min` fish in 2008 to `r annual.catches.fall.CO.old$max` fish in 2014 (mean = `r annual.catches.fall.CO$mean` $\pm$ `r round(annual.catches.fall.CO$SD,1)`). 
