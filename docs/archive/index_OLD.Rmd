...
```{r catch_dat.prep, echo=FALSE, warning=FALSE, message = FALSE, include = FALSE}
catch_table <- all_dat %>%  
...
## Catch by Year and Period ----
    ...
    for(i in spp){
        ...
        x <- all_dat %>%  
          ...
          catch_summary <- left_join(trap_effort2, catch_CT, by = c("Year", "Period"), suffix = c("","_CT")) %>%
...
spring.catch_summary <- catch_summary %>% filter(Period == "Spring") %>%
                                          select(!Period)

```

Since 2008, a total of `r format(sum(spring.catch_summary$nCT), big.mark =",")` juvenile Coastal Cutthroat Trout and `r format(sum(spring.catch_summary$nCO), big.mark =",")` juvenile Coho Salmon have been captured during spring trapping. Annual captures of CCT and CO have ranged from `r format(min(spring.catch_summary$nCT), big.mark =",")` to `r format(max(spring.catch_summary$nCT), big.mark =",")` and `r format(min(spring.catch_summary$nCO), big.mark =",")` to `r format(max(spring.catch_summary$nCO), big.mark =",")`, respectively, with overall means of `r format(mean(spring.catch_summary$nCT), digits = 1)` CCT and `r format(mean(spring.catch_summary$nCO), digits = 1)` CO.

```{r Spring.CatchSummaryTable, echo=FALSE, warning=FALSE, message = FALSE}
## Kable - Master Summary ----
options(knitr.kable.NA = '-')

kable(spring.catch_summary,
      "html",
      booktabs = T,
      ...
      col.names = c("Year", "Start", "End", "# Days Sampled",  
                    ...
                    "Med.","Min","Max", "n", "Mean", "SD" ,"Min","Max"),
      caption = "**Table 2. Spring capture summary - capture timing, total captures and length of Coastal Cutthroat Trout and Coho Salmon.**",
      digits = 1) %>%
...
```{r total.catch.plot.prep, echo=FALSE, warning=FALSE, message = FALSE, include=FALSE}
spring_catches <- all_dat %>%
              ungroup() %>%
              group_by(Year, Period, Species) %>%
              summarize(n = n()) %>%
...
total_springcatches <- ggplot(spring_catches) +
...
```{r total.catch.plot, echo=FALSE, warning=FALSE, message = FALSE}

total_springcatches

```

**`r fig_nums("total_springcatches", "Total number of Coastal Cutthroat Trout (CT) and Coho Salmon (CO) captured during spring trap operations from 2008 to 2024.")`**

```{r springCatch_prep ,echo=FALSE, warning=FALSE, message = FALSE}
    ## CCT  Data
    CCT_catch.prep <- all_dat %>% 
                 filter(Species == "CT", Period == "Spring")
    
    CCT_catch <- left_join(date_seq, CCT_catch.prep, by = c("Date", "Month")) %>%
    ...
    ## CO Catch
    CO_catch.prep <- all_dat %>% 
                 filter(Species == "CO", Period == "Spring")
    
    CO_catch <- left_join(date_seq, CO_catch.prep, by = c("Date", "Month")) %>%
                    mutate(Year = strftime(Date, format = "%Y"),
                           Species = "CO",
                           Period = "Spring",
                           Catch = ifelse(Length >0,1,0),) %>%
    ...
    Spring.Catch <- rbind(CCT_catch,CO_catch)
    
    
```

```{r passagedate, warning=FALSE, message = FALSE, echo=FALSE}
...
There is considerable interannual variability in the timing of downstream migrations. For Coastal cutthroat trout, the date that 50% of total out migrants passed the fence ranged from `r format(min(pass.date$minPass.std[pass.date$Species=="CCT"]), "%b-%d")` to `r format(max(pass.date$minPass.std[pass.date$Species=="CCT"]), "%b-%d")`(mean = `r format(mean(pass.date$mean.min[pass.date$Species=="CCT"]), "%b-%d")`). In contrast, the date that 50% of total out migrating Coho Salmon passed the fence ranged from `r format(min(pass.date$minPass.std[pass.date$Species=="CO"]), "%b-%d")` to `r format(max(pass.date$minPass.std[pass.date$Species=="CO"]), "%b-%d")`(mean = `r format(mean(pass.date$mean.min[pass.date$Species=="CO"]), "%b-%d")`)
