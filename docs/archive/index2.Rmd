...
Simms Creek is a small, 3rd order stream that flows into the Salish Sea near Campbell River's Willow Point neighborhood. The lower reach of Simms Creek (~3 km of channel, east of Jubilee Road), is contained within an urban green way that is bordered by residential properties. From Jubilee Road to the Inland Highway the stream is protected within [Beaver Lodge Lands](https://campbellriver.travel/news/forestry-guide-to-beaver-lodge-lands/), upstream of the Inland Highway, >1.5 km of tributary habitat is located within private forest lands. 

Historic fish observations show that Chum, Chinook, Pink and Coho Salmon have all been observed in the stream, as well as Coastal Cutthroat Trout, Rainbow Trout and Steelhead. Recent records suggest that Coho Salmon and Coastal Cutthroat Trout are the primary species within Simms Creek, both of which utilize the stream year round. [Greenways Land Trust](https://www.greenwaystrust.ca/) has supported Simms Creek Stewards for +20 years, including operation of a fish fence near the mouth of the stream since 2008. 

Each spring, all juvenile salmonids leaving Simms Creek are tallied. The objective of this project is to simply explore the juvenile dataset. All work is being done by myself, strictly for fun and out of curiosity. Any feedback, input, or support is welcome. Also, I have a soft spot for Coastal Cutthroat Trout, so this analysis will be trout heavy. 
...
```{original table, include = FALSE, echo=FALSE, warning=FALSE, message = FALSE }
CCT_table <-  simms_bio %>% 
                group_by(Year, Species) %>%
                dplyr::summarize(
...
```{r annual trap operation, echo=FALSE, warning=FALSE, message = FALSE}




as.Date('2008-06-15')-as.Date('2008-04-01')
